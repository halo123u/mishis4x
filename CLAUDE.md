...
**Matchmaking/lobby** (`be/matchmaking/`) is currently pure in-memory state on the handler struct — not persisted, won't survive a restart or multiple instances, despite a `game_matches` migration already existing for it. The API routes (`/api/lobbies`, `/api/lobbies/create`) exist and work, but there's no frontend UI calling them yet — `Home.tsx` is still a stub. This is known-incomplete, not a bug to silently "fix" by adding persistence unasked.
