// eBay's brand mark, from Simple Icons (simpleicons.org) - CC0-licensed
// SVG path data. Inlined directly rather than loaded from a CDN: the
// app's CSP only allows script/style from 'self' (plus Google Fonts), and
// a single small path is cheap enough that a network request/CSP change
// isn't worth it just to fetch it. Using the mark to link to a search on
// eBay's own site is the same nominative fair use every "Shop on eBay"
// button already relies on - not a claim of partnership or endorsement.
const EbayIcon = () => (
  <svg
    role="img"
    viewBox="0 0 24 24"
